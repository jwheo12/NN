...
기본적으로 프로그램은 아래 경로에 압축 해제된 MNIST 파일이 있다고 가정함 본인 환경 기준으로 바꾸셈.
...
기본 설정은 아래와 같음. 정확도에 쾌감 느끼면 하이퍼파라미터 튜닝해볼것
...
실행하면 에폭별 손실과 정확도, 그리고 테스트 이미지 몇 장의 ASCII 미리보기와 예측 확률을 출력.
